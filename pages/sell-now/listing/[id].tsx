...
export default function SellNowPage() {
  ...
  const [listItemPost, setListItemPost] = useState<ListItemPost>({
    ...
    more_info: listingDetail.more_info,
  } as ListItemPost);
  const [count, handlers] = useCounter(Number(listingDetail?.ask || 0), {
    min: Number(listingDetail?.ask || 0),
  });
            ...
            <div>
              ...
              <Group>
                <div className="inline-block text-[16px] px-[20px] py-[5px] bg-[#F1F1F1]">
                  Lowest Ask: ${Number(listingDetail?.lowest_ask || 0)}
                </div>
                <div className="inline-block px-[20px] text-[16px] py-[5px] bg-[#F1F1F1]">
                  Highest Offer: ${Number(listingDetail?.highest_offer || 0)}
                </div>
              </Group>
            </div>
            {listItemPost.condition === 'used' ? (
              <div className="w-screen mt-5 -ml-11 md:w-auto">
                <ProductCarousel images={listingDetail.images} />
              </div>
            ) : (
              <div className="w-screen mt-5 -ml-12 md:w-auto">
                <Image alt="product image" src={baseURL + '/' + (listingDetail?.images?.[0]?.filename || '')} />
              </div>
            )}
          </Grid.Col>
